...
├── ParameterizedMiddlewareInterface.php — extends MiddlewareInterface; handle(..., string ...$parameters) for `'name:arg1,arg2'` registrations
...
├── RouterInterface.php           — get/post/put/patch/delete route registration + toCache(); implemented by the framework Router, bound by RouterServiceProvider
...
Route registration (`get`/`post`/`put`/`patch`/`delete`, each returning the created route as `object`) plus `toCache()`, the serialisable list of controller routes. It is the slice of the router that modules need to expose an endpoint (`ez-php/health`, `ez-php/metrics`, `ez-php/graphql`, `ez-php/swagger-ui`) or to read the route table (`ez-php/openapi`) — without it, those five modules required all of `ez-php/framework` for one `EzPhp\Routing\Router` import. Matching, dispatch, groups and middleware stay framework-internal. The return type is `object` because `Route` is a framework class; the framework `Router` narrows it covariantly to `Route`.
...
| `Application`, `Container`, `Router` (implementations) | `ez-php/framework` |
